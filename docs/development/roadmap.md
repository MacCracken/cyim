...
| **`--regex=<flavor>` modifier on grep/replace verbs** | **Hard target before cyrius 5.7.x EOL (escalated from soft demand-gate 2026-04-27).** Upstream gate is the cyrius stdlib NFA module — small ABI: `re_compile` → handle, `re_match`, `re_free`; single flavor at first cut, likely POSIX ERE. Note: cyrius 5.7.13 ships a `lib/regex.cyr` *by name*, but it self-describes as "Simple glob-style matching + literal search. Not full regex." (surface: `glob_match`, `find_all`, `str_replace[_all]`) — that does **not** satisfy the cyim plan's contract. The NFA module must land before cyrius 5.7.x closes so cyim can consume it inside the same major series. (Bundled siblings — `--grepfiles` and `--context=N` — already shipped in v1.1.4.) | One addition: `--regex=<flavor>` modifier on `--grep` / `--grepfiles` / `--replace` / `--replace-all` — default stays literal substring (back-compat); parity across all four verbs is mandatory. Design agreed 2026-04-26 in cyrius-bb tooling-pain-points dogfood loop. Held until the cyrius stdlib NFA module lands (auditable-in-one-head + reuse across owl / agnoshi / etc. — lineage-level placement, not in-tree-in-cyim, not external Cyrius dep). When it ships, the cyim consumer side is mechanical: extend `_cli_match_at` (or split it) to dispatch literal vs compiled-regex, thread `--regex=<flavor>` through the four affected verb parsers (the v1.1.4 `--context=N` work already wired the duplicate-flag-guard machinery there), extend `tests/cli_smoke.sh` with regex cases, update `--help`. Patch-shaped (1.1.x), not minor. If 5.7.x looks likely to close before the NFA module is ready, surface the deadline risk back — don't let it slip silently to 5.8.x or 6.x. |
...
*Last updated: 2026-04-27 (v1.1.4 shipped: `--grepfiles` + `--context=N` + `--replace-files[-all]` + cyrius 5.7.13 toolchain bump; regex piece moved to its own demand-gated row, retained as hard pre-cyrius-5.7.x-EOL target).*
