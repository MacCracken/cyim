...
| **`--regex=<flavor>` modifier on grep/replace verbs** | *Shipped: v1.2.0 (ere), v1.3.0 (bre/re2/pcre/vim), v1.3.1 (fuzzy).* All six niyama flavors live across all six pattern verbs. Backreferences (`\1`-style) still deferred per niyama's long-term security-against-misuse plan. Surfaced in `cyim --help`. | v1.3.1 wired fuzzy by faking the missing `_search_at` (cstring-pointer arithmetic) and `_group_end` (compile-time pattern length stored in the `Matcher` struct's +24 slot). Substitute span is bounded-imperfect for insert / delete edits — eats `plen` source bytes regardless of actual match span — documented in `--help`. Tighten path: post-match span search via repeated `niyama_fuzzy_match` on candidate lengths, OR upstream `end_pos` exposure (the latter is the cleaner fix). Backref support and a cyim-side `--fuzzy-edits=<n>` knob (currently fixed at niyama's default `FUZZY_DEFAULT_K=2`) added when surface demand shows. |
...
*Last updated: 2026-05-06 (v1.3.1 shipped: `--regex=fuzzy` Levenshtein, default k=2, across all six pattern verbs. Substitute span is bounded-imperfect (± max_edits bytes) for non-substitution edits because niyama_fuzzy lacks `_group_end` — surfaced in `--help`. Closes the `--regex=<flavor>` rollout begun in v1.2.0.).*
