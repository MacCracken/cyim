...
| **Regex + grep ergonomics on agent-drive surface** | **Regex piece is upstream-gated:** cyrius stdlib ships a `lib/regex.cyr` module (math/yukti-shaped — small ABI: `re_compile` → handle, `re_match`, `re_free`; single flavor at first cut, likely POSIX ERE). The `--grepfiles` and `--context=N` pieces are not upstream-gated and could ship sooner, but bundled here to design the grep surface as one coherent expansion. | Three additions, designed together, shipped in the 1.1.x patch family: **(1) `--regex=<flavor>` modifier** on `--grep` / `--replace` / `--replace-all` — default stays literal substring (back-compat), parity across all three verbs is mandatory; design agreed 2026-04-26 in cyrius-bb tooling-pain-points dogfood loop; held until the cyrius stdlib module lands (auditable-in-one-head + reuse across owl / agnoshi / etc., not in-tree, not external Cyrius dep). **(2) `--grepfiles <pattern> <file...>` verb** — multi-file variant of `--grep`, accepts N positional file paths; output stays `FILE:N:LINE` (`grep -n` shape already disambiguates by filename, so multi-file is a natural extension); exit 0 on any match across all files, exit 1 on no match (matches `grep` exit semantics). **(3) `--context=N` modifier** on `--grep` / `--grepfiles` — emits N lines before+after each match (`grep -C N` shape); merges overlapping windows; `--` separator between non-adjacent groups per `grep` convention; symmetric only at first cut (defer asymmetric `--before` / `--after` until demand). All three modifiers stack orthogonally (`--regex` × `--context` × `--wc` all compose). Mechanical cyim-side work once regex lands: thread modifiers through the parser family, mirror the v1.1.3 duplicate-flag-guard pattern, extend `tests/cli_smoke.sh`, update `--help` for all four affected verbs. Patch-shaped (1.1.x), not minor. |
...
*Last updated: 2026-04-27 (regex demand-gated row expanded to bundle `--grepfiles <pattern> <file...>` multi-file verb and `--context=N` modifier — designed together with `--regex=<flavor>` as one grep-surface expansion; regex piece still gated on cyrius stdlib `lib/regex.cyr`).*
