...
| **`--regex=<flavor>` modifier on grep/replace verbs** | *Shipped in v1.2.0 (ere) and v1.3.0 (bre/re2/pcre/vim via niyama).* `fuzzy` queued for v1.3.1 — niyama_fuzzy lacks `_search_at`, so multi-match iteration needs a wrapper layer. Backreferences (`\1`-style) deferred per niyama's long-term security-against-misuse plan; not yet supported in any flavor as of v1.3.0. Both caveats surfaced in `cyim --help`. | v1.2.0 was design + ere; v1.3.0 added bre/re2/pcre/vim consuming the cyrius-5.9.0 niyama fold (`lib/niyama.cyr` via the sandhi explicit-`include` pattern). v1.3.1 plan: implement an iteration wrapper around `niyama_fuzzy_search` so `--replace-all --regex=fuzzy` can walk non-overlapping matches without `_search_at`. |
...
*Last updated: 2026-05-06 (v1.3.0 shipped: `--regex={bre,re2,pcre,vim}` via niyama 1.0.1 sandhi-folded into cyrius 5.9.0 stdlib; fuzzy gated to v1.3.1 with explicit deferred-diagnostic; backref support deferred per niyama's long-term security-against-misuse plan, surfaced in `--help`).*
