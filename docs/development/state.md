...
*Last bumped: 2026-05-06 (v1.3.2 — fuzzy substitute precision via post-match candidate-length walk + `--fuzzy-edits=<n>` modifier across six verbs + closeout pass. Newline preservation now correct on deletion edits; `--fuzzy-edits=0` enables exact-fuzzy. LSP client promoted to v1.4.0 since cyrius-lsp is stable.)*
