...
*Last bumped: 2026-05-06 (v1.3.5 — **plugin ABI proven end-to-end**. Four remaining fire-points (`status_segment`, `normal_key`, `ex_command`, `diagnostic_provider`) wired into render / mode-dispatch / command-parser paths. First working plugin: `src/plugins/trailing_ws.cyr` exercises 3 of 6 hooks. ABI freezes at ADR 0004 (1.3.6 plan).)*
