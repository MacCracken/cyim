...
*Last bumped: 2026-04-25 (v1.0.1 — agent-drive CLI flags)*
...
- **Last release**: `1.0.1` — agent-drive CLI surface (`--write`, `--replace`, `--replace-all`), 2026-04-25
- **Prior release**: `1.0.0` — first stable, 2026-04-25 (M0–M7 all landed)

## Binary

- `build/cyim` — DCE build size: **283,984 B** (v1.0.1 added `src/cli.cyr` for the agent-drive CLI surface; v1.0.0 was 274,656 B; M6 was 273,912 B; M5 was 262,504 B; M4 was 256,344 B; M3 was 226,064 B; M2 was 162,184 B; M1 was 101,560 B; M0 stub was 57,728 B).

## Tests

- Test suites: M0 + M1 (8) + M2 (4) + M3 (2) + M4 (4)
- Assertion count: 847 (M1 350; M2 +117 = 467; M3 +192 = 659; M4 +153 = 812; M6 +26 = 838; M7 +9 = 847)
- Integration smoke: `tests/integration_smoke.py` — 18 checks (14 PTY-driven + 4 headless via subprocess: `--headless`, `--write`, `--replace`, `--replace-all`)
...
ship in the binary). Remaining: system clipboard (via
`aethersafha`), LSP client (when cyrius-lsp stabilizes),
terminal embed, macros, etc.
...
| `daimon`-orchestrated agents | Surface ready (v1.0.1) | `cyim --headless` for full keymap drive; `cyim --write`/`--replace`/`--replace-all` for one-shot ops. Daimon integration when that consumer is ready. |
