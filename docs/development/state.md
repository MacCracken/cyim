...
- **VERSION**: `1.3.0`
- **Cyrius toolchain pin**: `5.9.1` (in `cyrius.cyml [package].cyrius`; unchanged from v1.2.2)
...
`cyrius` (5.9.1) → vendored stdlib in `lib/` (includes `regex.cyr` + `niyama.cyr` from 5.9.0+ fold + `unicode/*.cyr`) → `src/main.cyr` (explicit `include "lib/niyama.cyr"` per sandhi pattern) → `build/cyim`.
