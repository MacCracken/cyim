...
- `build/cyim` — DCE build size: TBD (M0 stub, will become a real metric at M1)

## Tests

- Test suites: `src/test.cyr` (scaffold smoke) + `tests/cyim.tcyr` (M0 smoke) + `tests/buffer.tcyr` (M1 bite 1: gap-buffer invariants) + `tests/roundtrip.tcyr` (M1 bite 2: file load/save round-trip) + `tests/tty.tcyr` (M1 bite 3: termios flag-mask + ANSI helpers) + `tests/dispatch.tcyr` (M1 bite 4: modal dispatch + headless drive)
- Assertion count: 166 (47 buffer + 23 round-trip + 37 tty + 57 dispatch + 2 smoke)
...
- **M0** — scaffold. *Done.*
- **M1 (in progress)** — gap-buffer + raw-mode TTY + modal dispatch. Sliced into 8 bites; bites 1 (gap-buffer), 2 (file round-trip), 3 (raw-mode TTY), and 4 (modal dispatch) landed. See [`roadmap.md`](roadmap.md).
