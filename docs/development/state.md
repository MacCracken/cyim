...
*Last bumped: 2026-05-06 (v1.3.4 — **plugin ABI scaffold** per ADR 0003. `src/plugin.cyr` lands the 6-hook registry + register/fire/lookup/collect API; `post_save` and `post_change` fire-points wired into core dispatch. No external plugin yet — scaffold compiles cleanly with `tests/plugin.tcyr` exercising the cycle (27 assertions). ABI provisional; freezes at ADR 0004 once the LSP forces design choices.)*
...
`cyrius` (5.9.13) → vendored stdlib in `lib/` (includes `regex.cyr` + `niyama.cyr` from 5.9.0+ fold + `unicode/*.cyr` properly packaged from 5.9.2; `args.cyr` heap-backed since 5.9.5; `fs.cyr` dir_list UAF fix in 5.9.6) → `src/main.cyr` (explicit `include "lib/niyama.cyr"` per sandhi pattern) → `build/cyim`.
