...
*Last bumped: 2026-08-23 (v1.9.3 — **closeout cut for the 1.9.x cycle**, all 11 CLAUDE.md steps run at P(-1) depth: [`docs/audit/2026-08-23-1.9x-closeout.md`](../audit/2026-08-23-1.9x-closeout.md). 2 findings, both fixed — the config read cap **fabricated a value** (a line cut mid-value applied a truncated number that landed back inside the valid range: `palette.keyword = 199999` became `19` and was painted), and 1.9.2's two precedence accessors had no caller anywhere, which architecture note 004 classifies as a coverage hole rather than dead code. Four itoa implementations counted and **deliberately not merged** — their bound policies are exactly what differs. **First closeout with no known-red gate**: `cyrius smoke` is 13/13 and gated. **1.9.x is closed; 1.10.0 opens clean, no open bugs.**)*
...
- **`cyrius tests`**: 21 suites, **1216 assertions** PASS, 0 failures *(measured at 1.9.3; +7 for the closeout's two findings, both mutation-tested. At 1.9.2: +9 for ADR 0005's config precedence, including a reversed-order control that would catch the precedence being backwards. At 1.9.0: +23 for `o` / `O` plus the `A` one-character-line regression, mutation-tested in three directions. At 1.8.5: +3 closing the `diag_msg` coverage hole. At 1.8.4: +13 for ADR 0006's save-path selection tests, which assert WHICH path ran rather than only the outcome. At 1.8.3: +25 from the hardening audit's regressions, every one mutation-tested against the pre-fix source. At 1.8.2: +4 openqasm routing, +3 the routing↔loading drift guard. The pre-1.8.1 "22 suites / 1150" counted `src/test.cyr`, an empty stub returning 0 — it asserts nothing)*
...
- ✅ **LSP smoke** (`tests/smcyr/lsp_fold.smcyr`): **13 passed, 0 failed** — was 4/9 from 2026-08-11 through 1.9.0. **BUG-002, closed at 1.9.1** by cyim-lsp 1.5.3: `_lsp_proc_exec` declared `var argv[4]` — four BYTES for four 64-bit pointers — so `lsp_client_start_default()`'s `("/usr/bin/env", "cyrius-lsp", 0)` wrote 24 bytes into 4, `execve` got a clobbered vector, and the child died on its own `sys_exit(127)`. Silent, because execve never took and so `env` was never there to complain. Root-caused from this repo at 1.8.2; fixed upstream; picked up as a tag bump with **no cyim source change**. **Now a CI gate** — its absence is why the feature sat dead for seven cuts. Full issue doc: [`issues/2026-08-11-lsp-fold-smoke-handshake.md`](issues/2026-08-11-lsp-fold-smoke-handshake.md).
...
- **`cyrius smoke` IS a CI gate as of 1.9.1** — added with BUG-002's fix. It needs `cyrius-lsp` on the runner, which the toolchain's `[release].bins` has been providing all along; the step asserts it explicitly so a packaging regression fails loudly rather than skipping. Still not gated: `cyrius audit` (its docs step is advisory).

## Cycle in flight

**The 1.9.x cycle is CLOSED at 1.9.3** — closeout complete, all 11 CLAUDE.md steps green at P(-1) depth ([audit](../audit/2026-08-23-1.9x-closeout.md)). The cycle ran: 1.9.0 `o` / `O` (plus the `A` one-character-line bug they uncovered) → 1.9.1 BUG-002 closed and `cyrius smoke` gated → 1.9.2 ADR 0005 decided and implemented → 1.9.3 closeout.

**First closeout with no known-red gate**, and **no open bugs** — BUG-001 closed at 1.3.3, BUG-002 at 1.9.1.

**1.10.0 opens next; nothing deferred blocks it.** No feature is claimed — cuts open as triggers surface (per [`roadmap.md`](roadmap.md)'s Post-1.5.x — Demand-Gated table). ⚠ ADR 0005 left a **live obligation**, not a deferral: every new `.cyimrc` key must be classified local-overridable or home-only when it is added. Everything genuinely deferred is in one table: [`roadmap.md` § Everything Still Deferred](roadmap.md).
...
**Closeout gate: satisfied.** The 1.9.x closeout shipped as 1.9.3. The next one is owed as the last patch of the 1.10.x line.
...
- **External Cyrius deps**:
  ...
  - `cyim-lsp` **1.5.3** — LSP client plugin; consumed via `lib/cyim-lsp.cyr` distfile, glue in `src/plugins/lsp_glue.cyr`. Self-contained bundle, own pin 6.5.18. 1.5.3 fixes BUG-002: `src/subprocess.cyr`'s `var argv[4]` / `var fallback[1]` were sized in pointer slots rather than bytes, so every spawn with an argument overran and `execve` failed silently. Picked up at cyim 1.9.1 with no cyim source change.
