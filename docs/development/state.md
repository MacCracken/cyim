...
*Last bumped: 2026-05-06 (v1.3.0 — `--regex=<flavor>` accepts `bre`/`re2`/`pcre`/`vim` via niyama 1.0.1 (sandhi-folded into cyrius 5.9.0 stdlib; pulled via explicit `include "lib/niyama.cyr"` per the cyrius-documented opt-in pattern). `fuzzy` recognized but parser rejects with v1.3.1-deferred message; backrefs deferred per niyama v1 security plan.)*
...
- **VERSION**: `1.3.0`
- **Cyrius toolchain pin**: `5.9.2` (in `cyrius.cyml [package].cyrius`; bumped from v1.2.2's 5.9.1 because 5.9.1's release tarball was missing `lib/unicode/*.cyr` — pre-fix subdir-nested stdlib propagation bug — which broke `cyrius deps` in CI for v1.3.0's niyama-fuzzy NFD-table consumption. 5.9.2 packages the unicode subdir correctly. Local builds were fine because `~/.cyrius/lib/unicode/` was present from a prior dev install; CI's clean tarball install hit the gap first.)
- **Last release**: `1.3.0` — `--regex=<flavor>` accepts four new engines via niyama 1.0.1: `bre` (POSIX-BRE), `re2` (Google RE2 — linear-time, no backrefs), `pcre` (PCRE-style — `\d`/`\w`/lookaround, no backrefs yet), `vim` (vim-regex with `\v`/`\V` mode prefixes). niyama consumed via explicit `include "lib/niyama.cyr"` (sandhi pattern) rather than `[deps].stdlib` auto-prepend; unicode normalize tables added to stdlib via v5.8.49 subdir-nested-resolution. `fuzzy` parser-recognized but rejected with `v1.3.1` diagnostic (niyama_fuzzy lacks `_search_at`, needs iteration wrapper). Backreferences deferred per niyama's long-term security-against-misuse plan; surfaced in `--help`. 10 new `cli_smoke.sh` cases (94→103 total). No cyrius toolchain bump, 2026-05-06
- **Prior release**: `1.2.2` — Cyrius toolchain bump 5.7.23 → 5.9.1 + version-string cleanup (`src/version_str.cyr` auto-generated by `scripts/version-bump.sh` so `cyim --version` can never silently drift again). niyama 1.0.1 fold trigger fired at cyrius 5.9.0 so `lib/niyama.cyr` is now vendored stdlib alongside `lib/regex.cyr`, 2026-05-06
- **Prior prior**: `1.2.1` — interactive-mode fixes: Enter key in INSERT now splits the line via `ACT_INSERT_NEWLINE` (was inserting CR byte 13 verbatim); arrow keys parse CSI sequences in `editor_feed` (driver-side) instead of being dispatched byte-by-byte through editor_dispatch, 2026-04-28

## Binary

- `build/cyim` — DCE build size: **888,408 B** (v1.3.0 niyama fold + 4 engine flavors; +518,640 B over v1.2.2's 369,768 B = niyama dist (~6.6 KLOC of engine code: bre/re2/pcre/fuzzy/vim engines + posix_classes + unicode_props shared shims) plus the unicode normalization tables (`lib/unicode/_normalize_data.cyr` is ~300 KB on its own) it pulls in even though fuzzy isn't exposed yet (niyama is one concatenated artifact). The flat 4-engine dispatch in `src/cli.cyr` (FLAVOR_BRE/RE2/PCRE/VIM constants, `_re_search_at` / `_re_search` / `_re_group_end` helpers, `_flavor_validate` parser arm) adds <2 KB. v1.3.1 fuzzy expansion expected to add ~0 incremental binary (engine already linked, tables already loaded). v1.2.2 was 369,768 B (toolchain bump 5.7.23 → 5.9.1 + version-string cleanup; +13,344 B over v1.2.1's 356,424 B = +13,264 B stdlib drift between the two cyrius releases plus +80 B for the `src/version_str.cyr` extraction (single-source-of-truth for `cyim --version`, regenerated by `scripts/version-bump.sh` per the cyrius pattern). v1.2.1 added `editor_feed` + 8-byte read buffer in `run_editor`/`run_headless` for driver-side CSI parsing, +1,168 B over v1.2.0's 355,256 B; v1.2.0 added the Matcher + RegexOpts abstractions, `_cli_count_matches_m` / `_cli_substitute_regex` / `_cli_substitute_m` matcher-dispatching helpers, six `_dispatch_<verb>(ac)` extraction functions, and the cyrius stdlib `regex` dep — Pike NFA engine consumed via `regex_compile`/`regex_search`/`regex_search_at`/`regex_group_*`. The bulk of the +44,336 B delta from 1.1.4 is the engine itself; cyim consumer code adds ~4 KB; the dispatch extraction is byte-neutral. v1.1.4 was 312,088 B; v1.1.3 was 300,640 B with 8 duplicate-flag guards across four verbs in `src/main.cyr` + a slightly longer `--grep` help string; v1.1.2 was 298,392 B with `run_batch` + `_cli_drain_stdin_raw` in `src/cli.cyr` and `--batch` dispatch in `src/main.cyr`; v1.1.0 was 293,192 B; v1.0.2 was 283,984 B; v1.0.0 was 274,656 B; M6 was 273,912 B; M5 was 262,504 B; M4 was 256,344 B; M3 was 226,064 B; M2 was 162,184 B; M1 was 101,560 B; M0 stub was 57,728 B).
...
- CLI parser smoke: `tests/cli_smoke.sh` — 103 PASS assertions across 84 cases (v1.3.0 added 10 cases for the four niyama flavors: 1 grep per flavor on a flavor-distinctive idiom (BRE `[0-9][0-9]*` vs ERE `+`; PCRE `\d`; vim `\+`), `--replace-all --regex=pcre` substitute-path flavor dispatch, `--replace --regex=re2 --expect-1` count-path flavor dispatch, fuzzy-deferred gate asserting the v1.3.1 diagnostic, BRE invalid-pattern → exit 2, multi-file `--grepfiles --regex=pcre`, `--replace-all --regex=vim` with file-sourced OLD/NEW). Existing case 64 ("`--regex=pcre` unknown flavor") repurposed to `--regex=foobar` since pcre is now a real flavor. (10 v1.1.1 interspersed-modifier regressions + 18 v1.1.2 `--batch` cases incl. atomicity, malformed-stdin, post-save assertions, em-dash unicode round-trip + 7 v1.1.3 cases (6 duplicate-flag refusals across the four verbs + a `--grep '^foo'` literal-substring regression) + 17 v1.1.4 cases: 5 `--grepfiles` (single/multi-file match, no-match exit 1, missing FILE exit 3, FILE:N:LINE shape), 6 `--context=N` (pre/match/post emit shape, overlapping-window merge, non-adjacent `--` separator, `--context=0` byte-for-byte back-compat regression guard, `--context=<non-int>` rejection, duplicate `--context=` refusal), and 6 `--replace-files[-all]` (multi-line OLD/NEW round-trip, empty OLD_FILE rejection, missing OLD_FILE exit 3, non-unique OLD exit 5, atomicity guard on `--expect-1` mismatch, `--wc=l` output shape) + 16 v1.2.0 cases: `--regex=ere` per-verb basics (digits / alternation / anchors), literal-default back-compat regression guard, error paths (invalid pattern / unknown flavor / empty flavor / duplicate `--regex=`), multi-file regex via `--grepfiles`, regex × file-sourced OLD/NEW via `--replace-files[-all]`, and three composition cases (`--regex × --context`, `--regex × --wc=l`, `--regex × --expect-1`))
...
`cyrius` (5.9.2) → vendored stdlib in `lib/` (includes `regex.cyr` + `niyama.cyr` from 5.9.0+ fold + `unicode/*.cyr` properly packaged in 5.9.2 tarball) → `src/main.cyr` (explicit `include "lib/niyama.cyr"` per sandhi pattern) → `build/cyim`.
