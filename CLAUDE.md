...
- **`daimon`-orchestrated agents** — AI assistants (Claude-style and AGNOS-native agents alike) drive cyim programmatically. The same modal surface humans use, agents drive headlessly. The edit loop closes through cyim — nothing in the loop ships from outside the library. Six CLI surfaces serve this consumer:
  ...
  6. `cyim --batch <file>` — N substitutions in one call. Stdin format: NUL-separated alternating tokens `OLD1\0NEW1\0OLD2\0NEW2\0…\0`. Pairs apply sequentially in memory; one save at the end. Default per-pair semantics match `--replace` (OLD unique at apply time); `--all` switches every pair to `--replace-all`. Atomic — failure mid-batch leaves FILE untouched on disk, for both kinds of failure: a pair whose OLD is not unique aborts the batch before any write, and since 1.8.4 a *write* failure leaves the original intact too (sibling temp + `rename`, per [ADR 0006](docs/adr/0006-atomic-save.md)). The exception is the enumerated in-place set — symlinks, hardlinked files, non-regular files, files owned by another uid, non-writable directories, and agnos — where renaming would break something the user relies on more; there a failed write still truncates, and is reported.
